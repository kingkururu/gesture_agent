...
public:
    ...
    static AsyncLogger& getInstance() {
        static AsyncLogger instance; // Guaranteed to be destroyed at the end of the program
        return instance;
    }

    // Explicitly delete copy constructor and assignment (Standard Singleton practice)
    ...
    void shutdown() {
        stop_thread_ = true;
        log_queue_.stop(); // Wake up the thread so it can exit
        if (logging_thread_.joinable()) {
            logging_thread_.join();
        }
    }
...
void log_info(const std::string& message) {
    AsyncLogger::getInstance().log(message, spdlog::level::info);
}

void log_warning(const std::string& message) {
    AsyncLogger::getInstance().log(message, spdlog::level::warn);
}

void log_error(const std::string& message) {
    AsyncLogger::getInstance().log(message, spdlog::level::err);
}
